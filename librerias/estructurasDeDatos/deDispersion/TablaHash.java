...
public class TablaHash<C, V> implements Map<C, V> {
	...
	return (double)this.talla / (double)elArray.length;
    ...
    public ListaConPI<C> claves() {
        ...
        ListaConPI<EntradaHash<C, V>> auxList;
        for(int i = 0; i < elArray.length; i++){
            auxList = elArray[i];
            for(auxList.inicio(); !auxList.esFin(); auxList.siguiente()){
                lista.insertar(auxList.recuperar().clave);
            }
        }
	...
	ListaConPI<EntradaHash<C, V>>[] elArrayAntiguo = this.elArray;
        ...
        /*
        OJO, antes de redispersar hay que actualizar la referencia
        this.elArray, pues afecta a indiceHash(e.clave)
        */
        ...
        /* Faster method, eliminating things that cannot happen */
        /*
        ListaConPI<EntradaHash<C, V>> l = elArray[indiceHash(e.clave)];
        l.fin();    // Alternativamente, this.inicio();
        l.insertar(new EntradaHash<C, V>(e.clave, e.valor));
        talla++;
        */
