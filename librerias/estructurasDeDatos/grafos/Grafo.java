...
public abstract class Grafo {
    ...
    public Arista[] arbolRecubrimientoBFS() {
        ...
        return ( ordenVisita == (numVertices() - 1) ) ? res : null;
    }
    protected void arbolRecubrimientoBFS(int origen, Arista[] res){
        ...
        while(!q.esVacia()){
            int u = q.desencolar().intValue();      // origen
            ListaConPI<Adyacente> lista = adyacentesDe(u);
            for(lista.inicio(); !lista.esFin(); lista.siguiente()){
                Adyacente a = lista.recuperar();    // destino
                if(visitados[a.destino] == 0){
    ...
    public Arista[] kruskal() {       
        ...
        Arista[] res = new Arista[numVertices() - 1]; // Array resultado
        UFSet mfset = new ForestUFSet(numVertices()); // Sets
        /* Cola prioridad para guardar aristas en orden */
        MonticuloBinarioR0<Arista> aristasFactibles = new MonticuloBinarioR0<Arista>();
        ...
        int resIndex = 0;   
        while(resIndex < (numVertices() - 1) && !aristasFactibles.esVacia()){
            
            Arista a = aristasFactibles.eliminarMin();
            int r1 = mfset.find(a.origen);
            int r2 = mfset.find(a.destino);
            if(r1 != r2){
                /* 
                 Shouldn't be neccesary to use the representant,
                 but the mfset class may be wrong, so use it.
                */
                mfset.union(r1, r2);
                res[resIndex++] = a;
            }
            //System.out.println("resIndex: " + resIndex + " res.length: " + res.length);
        }
        return ( resIndex == (numVertices() - 1) ) ? res : null;
