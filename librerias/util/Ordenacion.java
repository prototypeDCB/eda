...
public class Ordenacion {
    ...
    */
    ...
    private static <T extends Comparable<T>> T[] mergeSort2(T[] v,
        ...
        /*COMPLETAR*/
        if(i >= f){
            T[] v2 = (T[]) new Comparable[1];
            v2[0] = v[i];   // REMEMBER: array goes from i to f
            return v2;
        }else if(f == i + 1){
            T[] v2 = (T[]) new Comparable[2];
            if(v[i].compareTo(v[f]) <= 0){   // if ordered
                v2[0] = v[i];   // REMEMEBER: array goes from i to f, not 0 to 1
                v2[1] = v[f];
            } else {
                v2[1] = v[i];
                v2[0] = v[f];
            ...
            int m = (i + f) / 2;
            T[] arrLeft = mergeSort2(v,i,m);
            T[] arrRight = mergeSort2(v,m+1,f); 
            
           return merge2(arrLeft, arrRight);
        }
    }        
    ...
    private static <T extends Comparable<T>> T[] merge2(T[] v1, T[] v2) {
        ...
        int v2Index = 0;
        int auxIndex = 0;
        T[] auxArr = (T[]) new Comparable[v1.length + v2.length];
        
        while(v1Index < v1.length && v2Index < v2.length){
            if(v1[v1Index].compareTo(v2[v2Index]) < 0){
                auxArr[auxIndex++] = v1[v1Index++];
            } else{
                auxArr[auxIndex++] = v2[v2Index++];
            }
        }
